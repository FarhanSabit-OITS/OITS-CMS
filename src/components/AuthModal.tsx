...
export default function AuthModal({ onSuccess, onClose }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mfaRequired) {
        ...
        const data = await response.json();
        if (response.ok && data.success) {
          onSuccess({ ...data.user, twoFactorVerified: true });
        } else {
  ...
  return (
    <div className="flex flex-col max-w-md w-full bg-white border border-slate-200 rounded-2xl shadow-xl p-6 overflow-hidden">
          ...
          <>
            ...
            {isLogin && (
              <div className="text-center font-sans text-[9px] text-slate-450 pt-1 leading-normal font-semibold">
                ...
                <span className="text-slate-500">Standard user (Dashboard config + MFA optional)</span>: <span className="text-blue-600 font-bold">'user'</span> / <span className="text-blue-600 font-bold">'user123'</span>
              </div>
            )}
